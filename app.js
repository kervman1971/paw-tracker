...
function undoLastAction() {
  ...
  if (action.type === 'activity') {
    ...
    const idx = list.findIndex(e => (action.entry.fbKey && e.fbKey === action.entry.fbKey) || e.ts === action.entry.ts);
    if (idx !== -1) {
...
function showToast(msg, canUndo = false) {
  ...
  if (canUndo) {
    toast.innerHTML = `${msg} <button class="toast-undo-action" style="background:none;border:none;color:#FCD34D;font-weight:900;cursor:pointer;margin-left:8px;text-decoration:underline;">↺ Undo</button>`;
